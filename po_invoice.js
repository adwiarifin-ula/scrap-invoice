...
(async () => {
    ...
    // for (let day of range.by('day')) {
        ...
        let lastPage = process.env.END_PAGE;
        let totalPage = 1;
        do {
            // refresh token
            ...
            let poResults = await supplierService.getPurchaseOrders(params);
            while (!poResults) {
                // maybe got 401
                await refreshToken();
                poResults = await supplierService.getPurchaseOrders(params);
            }
            const { data, page } = poResults;
